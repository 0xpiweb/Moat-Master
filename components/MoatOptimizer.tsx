...
export default function MoatOptimizer() {
  ...
  const [epochRewards,      setEpochRewards]      = useState(28.10)
  const [epochInput,        setEpochInput]        = useState('28.10')
  ...
  const fetchDeposit = useCallback(async () => {
    try {
      ...
      // 28.10 default when the true epoch pool is higher.
  ...
  const DAILY_POOL       = 28.10 / 14  // 2.007 AVAX — bi-weekly epoch pool ÷ 14 days
